...
public class Dj extends Hero implements IHeroJob{
    public Dj(String name, int hp, int atk, int def, int agi){
    ...
    public AttackResult specialAttack(CharacterBase target) {
        System.out.println(this.getName() + "が曲を流した!");
        ...
        if (new Random().nextInt(100) >= successRate) { // 失敗判定
            ...
            System.out.println("選曲が悪く、" + target.getName() + "には刺さらなかったようだ…");
            return ret;
        }

        // 攻撃成功時は常に4倍のダメージ
        damage *= 4;
        System.out.println("選曲が" + target.getName() + "にブッ刺さった!ダメージが4倍に!");
